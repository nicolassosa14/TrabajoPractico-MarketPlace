...
export class SupabaseAddressRepository implements AddressRepository {
  ...
  async createAddress(address: Address) {
    const verify = await this.VerifyExistAddressForUserIDandStreetAddress(
      address.getUser_id(),
    ...
    const { data, error } = await this.supabaseClient.from('addresses').insert({
  ...
  async VerifyExistAddressForUserIDandStreetAddress(
    user_id: string,
    ...
    if (error)
      throw new BadRequestException(
        'Error al verificar la dirección: ' + error.message,
      );
  ...
  async EditAdressByID(address: Address): Promise<any> {
    ...
    if (error)
      throw new BadRequestException(
        'Error al editar la direccion: ' + error.message,
      );
  ...
  async deleteAddress(user_id: string, id: string): Promise<any> {
    ...
    if (error)
      throw new BadRequestException(
        'Error al eliminar la direccion: ' + error.message,
      );
