...
export class SupabaseLogisticRepository implements LogisticRepository {
    ...
    async createLogistic(logistic : Logistic) {
        ...
        if (error) throw new BadRequestException('Error al crear el logistic: ' + error.message);
    ...
    async VerifyUserIdDriver(user_id:string) : Promise<any> {
        const { data, error } = await this.supabaseClient
            ...
            .select('*')
            .eq('user_id', user_id)
        if (error) throw new BadRequestException('Error al verificar el driver: ' + error.message);
        return data;
    }
    async findAllAvailableLogistics(): Promise<Logistic[]> {
        ...
        if (error) throw new BadRequestException('Error al obtener los logistics: ' + error.message);
        if (data.length === 0) {
    ...
    async UpdateStatusLogisticByID(id:string, user_id:string, is_available:boolean): Promise<any> {
        const { data, error } = await this.supabaseClient
            .from('drivers_details')
    ...
    async findLogisticByID(id:string): Promise<Logistic> {
        ...
        const { data, error } = await this.supabaseClient
            ...
            .eq('id', id)
            .single();
        if (error) throw new BadRequestException('Error al obtener el logistic: ' + error.message);
        return data;
    }
